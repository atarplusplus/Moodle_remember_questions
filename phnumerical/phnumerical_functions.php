 ...
 */
class qtype_phnumerical_functions {
 ...
 */
	function generate_vars($question) {
		...
		$params = get_records('ph_params', 'question', $id);
		$vars = '';

		foreach ($params as $param) {
			//becouse it can be 'son' and the vars declare in another question
			ob_start();
			eval($vars); //evaluate declaration string that
			ob_end_clean();
			...
			ob_start();
			eval('$vars.=' . $param->value. ';');
			ob_end_clean();

			//Check if the value of $param->value is not null, to avoid the parsing errors in the question preview
			$vars .= ';';
		}
	...
	function strip_math_html($formula, $floats, $vars, $questionid, $attemptid) {
		...
		self::strip_html($formula); //not necessary since the 'mathExpression' class affects style only in the HTML editor
		return $formula;
	...
	function update_question_params(&$question) {
		...
		while ($v = optional_param('v' . $i)) { //Get the posted params
			...
			if (isset($oldparams [$param->id])) { //if already in db update
	...
	//save old_id to know who is father and who is son
